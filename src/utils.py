...
def get_bypassflare_version() -> str:
    global BYPASSFLARE_VERSION
    if BYPASSFLARE_VERSION is not None:
        return BYPASSFLARE_VERSION
    ...
    with open(package_path) as f:
        BYPASSFLARE_VERSION = json.loads(f.read())['version']
        return BYPASSFLARE_VERSION
...
def find_chrome_executable():
    ...
    for candidate in candidates:
        logging.debug('checking if %s exists and is executable' % candidate)
        if os.path.exists(candidate) and os.access(candidate, os.X_OK):
            logging.debug('found! using %s' % candidate)
